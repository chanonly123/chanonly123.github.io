...
$(document).ready(function () {
	...
	function convert_to_swift_string_const(text) {
	...
	function convert_to_swift_UIColor_const(text) {
		text = text.replace("<resources>", "");
		text = text.replace("</resources>", "");
		text = text.replace(/<color name=\"/g, "static let ");
		text = text.replace(/<\/color>/g, ")");
		text = text.replace(/">#/g, " = UIColor.colorFromCode(code: 0x");
		return text;
	}
	...
	var dest = convert_to_swift_UIColor_const(src);
	$("#textarea2").val(dest);

	$("#button1").click(function () {
		var src = $("#textarea1").val();
		var dest = convert_to_swift_string_const(src);
		$("#textarea2").val(dest);
	});
